...
saves a tensor and label vector in npy format.  This version is for cases with 4 end members.
'''
...
def GetPercentMixClass(CLS, UL, LR):# mixed classes, if more than 2, ignore that patch.
    ...
    if (c.size>0):
        if np.min(c)>0:
            ...
            if 3 in c:
                C4 = np.where(c==4)
                ClassOut[0,0,3] = counts[C4]/np.sum(counts)
    else:
        ClassOut[0,0,0] = -1
